...
export class UnifiedPredictionService {
  ...
  async getPredictions(timeRange = "week") {
    try {
      // Return cached predictions based on time range
      const now = Date.now();
      const timeRanges = {
        day: 24 * 60 * 60 * 1000,
      ...
      const cutoffTime = now - (timeRanges[timeRange] || timeRanges.week);

      // Filter active predictions by time range
      const filteredPredictions = Array.from(
        this.activePredictions.values(),
      ).filter(
        (prediction) =>
          prediction.timestamp && prediction.timestamp >= cutoffTime,
      );
