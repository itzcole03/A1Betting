...
export class BettingStrategyService {
            ...
            for (const [marketName, books] of markets) {
                ...
                if (totalImpliedProb < 1) {
                    ...
                    opportunities.push({
                        ...
                        event: null, // Will be populated by the caller
                        selection: null, // Will be populated by the caller
                        odds: null, // Will be populated by the caller
                        prediction: 0,
    ...
    findMiddleOpportunities(markets) {
        const opportunities = [];
        try {
            for (const [marketName, books] of markets) {
                this.validateMarket(marketName, books);
                ...
                for (let i = 0; i < sortedSpreads.length - 1; i++) {
                    ...
                    if (middleSize >= 1) {
                        ...
                        const ev = this.calculateMiddleEV(middleSize, sortedSpreads[i].book.odds, sortedSpreads[i + 1].book.odds);
                        if (ev > 0) {
                            opportunities.push({
    ...
    optimizeTeaserStrategy(legs, points) {
        try {
    ...
    findBestOdds(books) {
        return books.reduce((acc, book) => {
            const existingBook = acc.find(b => Math.abs(this.oddsToSpread(b.odds) - this.oddsToSpread(book.odds)) < 0.5);
    ...
    calculateTotalImpliedProbability(books) {
    ...
    calculateMiddleEV(middleSize, odds1, odds2) {
    ...
    addHistoricalData(market, odds) {
        const existing = this.historicalData.get(market) || [];
        this.historicalData.set(market, [...existing, odds]);
    }
    getHistoricalData(market) {
        return this.historicalData.get(market) || [];
    }
    createOpportunity(type, marketName, books, expectedValue, confidence, metadata = {}, sport = 'UNKNOWN', gameId = uuidv4(), description = `${type} opportunity in ${marketName}`) {
        return {
            id: uuidv4(),
            sport,
            gameId,
