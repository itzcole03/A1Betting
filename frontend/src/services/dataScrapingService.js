 ...
 * }
 */
export const fetchDailyFantasyProjections = async (date, league) => {
  const trace = unifiedMonitor.startTrace(
    "dataScrapingService.fetchDailyFantasyProjections",
    "http.client",
  );
  try {
    ...
    const errContext = {
      ...
      operation: "fetchDailyFantasyProjections",
      date,
      league,
    };
    ...
    throw new AppError(
      "Failed to fetch daily fantasy projections from backend.",
      undefined,
      errContext,
      error,
    );
 ...
 * }
 */
export const triggerScrapingJob = async (job_type, league, target_date) => {
  const trace = unifiedMonitor.startTrace(
    "dataScrapingService.triggerScrapingJob",
    "http.client",
  );
  try {
    ...
    const response = await post(endpoint, null); // Sending null body as params are in URL
    if (trace) {
      trace.setHttpStatus(response.status);
      unifiedMonitor.endTrace(trace);
    }
    ...
    if (error instanceof APIError || error instanceof AppError) throw error;
    throw new AppError(
      "Failed to trigger scraping job on backend.",
      undefined,
      errContext,
      error,
    );
...
const mapBackendDFPToFrontend = (backendItem) => {
  return {
