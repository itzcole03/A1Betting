...
export const useUnifiedAnalytics = (config = {}) => {
    ...
    const mlQuery = useQuery({
        ...
        queryFn: async () => {
            if (!analyticsService)
                throw { message: 'Analytics service unavailable' };
            try {
                ...
                const errorMessage = error instanceof Error ? error.message : 'Failed to fetch ML analytics';
                if (errorService)
    ...
    const bettingQuery = useQuery({
        ...
        queryFn: async () => {
            ...
            try {
                ...
                const errorMessage = error instanceof Error ? error.message : 'Failed to fetch betting analytics';
                if (errorService)
                    errorService.handleError(new Error(errorMessage), {
