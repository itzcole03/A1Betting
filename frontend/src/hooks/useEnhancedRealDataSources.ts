...
export function useEnhancedRealDataSources() {
  ...
  const connectToSources = useCallback(async () => {
    ...
    try {
      ...
      if (!hasApiKeys) {
        ...
        setDataReliability(0.8);

        // Initialize demo sources
        const demoSources =
          await enhancedDataSourceManager.initializeAllSources();
        setDataSources(demoSources);

        // Process demo data
        const demoGames = dataProcessor.processGames(
          convertToRealDataSources(demoSources),
        );
        const demoPlayers = dataProcessor.processPlayers(
          convertToRealDataSources(demoSources),
        );
        ...
        console.log(
          `🎮 Demo mode initialized: ${demoGames.length} games, ${demoPlayers.length} players`,
        );
        return;
      }
      ...
      setConnectionStatus(
        `Connected to ${connectedSources.length} enhanced data sources`,
      );
      setLastUpdate(new Date());

      // Process the enhanced data
      const processedGames = dataProcessor.processGames(
        convertToRealDataSources(sources),
      );
      const processedPlayers = dataProcessor.processPlayers(
        convertToRealDataSources(sources),
      );
  ...
  const convertToRealDataSources = (
    sources: Map<string, EnhancedDataSource>,
  ) => {
  ...
  const getPlayersForSport = (sport: string): any[] => {
    ...
    return (
      players[sport] || [{ name: "Demo Player", team: "DEMO", position: "P" }]
    );
  };
  ...
  return {
    ...
    connectedSourcesCount:
      enhancedDataSourceManager.getConnectedSources().length,
...
const dataProcessor = {
  processGames: (sources: any) => [],
  processPlayers: (sources: any) => [],
};
