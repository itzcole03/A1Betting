...
import { getPlayers, submitLineup } from '@/services/api';
...
export function useLineupAPI() {
    ...
    const filterPlayers = useCallback(({ position, team, minSalary, maxSalary, minConfidence, searchTerm, } = {}) => {
        return players.filter(player => {
            if (position && player.position !== position)
                return false;
            if (team && player.team !== team)
                return false;
            if (minSalary && player.salary < minSalary)
                return false;
            if (maxSalary && player.salary > maxSalary)
                return false;
            if (minConfidence && player.confidence < minConfidence)
                return false;
