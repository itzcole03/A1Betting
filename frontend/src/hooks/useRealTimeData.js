...
export const useRealTimeData = () => {
    ...
    useEffect(() => {
        const initializeData = async () => {
            ...
            const connectedSources = mockDataSources.filter((source) => source.status === "connected");
            const avgQuality = connectedSources.reduce((sum, source) => sum + source.quality, 0) /
                connectedSources.length;
            const avgReliability = connectedSources.reduce((sum, source) => sum + source.reliability, 0) /
                connectedSources.length;
            setState((prev) => ({
                ...prev,
        ...
        const interval = setInterval(() => {
            setState((prev) => {
                ...
                const updatedPredictions = prev.predictions.map((pred) => ({
                    ...pred,
                    confidence: Math.max(0.5, Math.min(0.95, pred.confidence + (Math.random() - 0.5) * 0.05)),
                }));
                ...
                const updatedSources = prev.dataSources.map((source) => {
                    if (Math.random() < 0.1) {
                        ...
                        return {
                            ...source,
                            lastUpdate: source.status === "connected" ? Date.now() : source.lastUpdate,
                            quality: source.status === "connected"
                                ? Math.max(0.6, Math.min(0.98, source.quality + (Math.random() - 0.5) * 0.03))
                                : source.quality,
                        };
                    }
                    return source;
                });
                const connectedSources = updatedSources.filter((source) => source.status === "connected");
                const avgQuality = connectedSources.length > 0
                    ? connectedSources.reduce((sum, source) => sum + source.quality, 0) / connectedSources.length
                    : 0;
                return {
                    ...prev,
    ...
    const getTopPredictions = useCallback((limit = 10) => {
        return state.predictions
            .sort((a, b) => b.confidence - a.confidence)
            .slice(0, limit);
    }, [state.predictions]);
    // Get predictions by sport
    const getPredictionsBySport = useCallback((sport) => {
        return state.predictions.filter((pred) => pred.sport.toLowerCase() === sport.toLowerCase());
    }, [state.predictions]);
    // Get data source by type
    const getDataSourcesByType = useCallback((type) => {
    ...
    return {
        ...
        activePredictionsCount: state.predictions.filter((p) => p.status === "active").length,
    };
};
