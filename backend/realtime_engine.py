...
from ensemble_engine import ultra_ensemble_engine, PredictionContext
...
class StreamAggregator:
    ...
    async def process_message(self, message: StreamMessage) -> Optional[StreamMessage]:
        ...
        try:
            ...
            # Aggregate messages
            aggregated = await self._aggregate_messages(buffer_messages, rule["merge_strategy"])
            ...
            logger.error(f"Message aggregation failed: {str(e)}")
    ...
    async def _aggregate_best_odds(self, messages: List[StreamMessage]) -> StreamMessage:
    ...
    async def _aggregate_line_movement(self, messages: List[StreamMessage]) -> StreamMessage:
        ...
        if len(sorted_messages) > 1:
            ...
            latest.data["movement_direction"] = "up" if movement > 0 else "down" if movement < 0 else "stable"
    ...
    async def _aggregate_accumulate(self, messages: List[StreamMessage]) -> StreamMessage:
    ...
    def _initialize_trigger_rules(self) -> Dict[StreamType, List[Dict]]:
        ...
        return {
            ...
            StreamType.INJURY_ALERTS: [
                {
                    "condition": lambda msg: msg.data.get("severity", "minor") in ["major", "questionable"],
            ...
            StreamType.LINE_MOVEMENTS: [
                {
                    "condition": lambda msg: abs(msg.data.get("movement_magnitude", 0)) > 2.0,
            ...
            for rule in rules:
                try:
                    if rule["condition"](message):
                        ...
                        cooldown = self.prediction_cooldowns.get(rule["trigger_type"], 60)
                    ...
                    logger.warning(f"Trigger evaluation failed: {str(e)}")
                    continue
...
class RealTimeStreamManager:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.error(f"Stream manager initialization failed: {str(e)}")
            raise
    ...
    async def _setup_redis_subscriptions(self):
        ...
        try:
            ...
            logger.error(f"Redis subscription setup failed: {str(e)}")
            ...
            async for message in pubsub.listen():
                if message["type"] == "message":
                    try:
                        ...
                        logger.warning(f"Redis message processing failed: {str(e)}")
                        
        except Exception as e:
            logger.error(f"Redis message handler error: {str(e)}")
    ...
    async def _message_processor(self):
        ...
        try:
            ...
            logger.error(f"Message processor fatal error: {str(e)}")
    ...
    async def _process_stream_message(self, message: StreamMessage):
        ...
        try:
            ...
            triggers = await self.prediction_trigger.evaluate_triggers(aggregated_message)
            ...
            logger.error(f"Stream message processing failed: {str(e)}")
    ...
    async def _handle_prediction_trigger(self, trigger: Dict[str, Any]):
        ...
        try:
            ...
            logger.error(f"Prediction trigger handling failed: {str(e)}")
    ...
    async def _get_event_features(self, event_id: str) -> Optional[Dict[str, float]]:
        ...
        try:
            ...
            logger.error(f"Feature retrieval failed for event {event_id}: {str(e)}")
            return None
    ...
    async def _broadcast_message(self, message: StreamMessage):
        ...
        try:
            ...
            for subscriber_id, subscription in self.subscribers.items():
                try:
                    ...
                    if subscription.websocket:
                        await self._send_websocket_message(subscription.websocket, message)
                    ...
                    logger.warning(f"Failed to send message to subscriber {subscriber_id}: {str(e)}")
            ...
            logger.error(f"Message broadcast failed: {str(e)}")
    ...
    def _message_matches_filters(self, message: StreamMessage, filters: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.warning(f"Filter matching failed: {str(e)}")
    ...
    async def _statistics_updater(self):
        ...
        try:
            ...
            logger.error(f"Statistics updater error: {str(e)}")
    ...
    async def _cleanup_task(self):
        ...
        try:
            while True:
                ...
                for key, messages in list(self.stream_aggregator.message_buffer.items()):
                    # Remove messages older than 1 hour
                    while messages and (current_time - messages[0].timestamp).total_seconds() > 3600:
                        messages.popleft()
    ...
    async def unsubscribe(self, subscriber_id: str) -> bool:
        ...
        try:
            if subscriber_id in self.subscribers:
                ...
                if subscription.websocket and subscription.websocket in self.websocket_connections:
