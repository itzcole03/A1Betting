...
from data_sources import ultra_data_manager, DataType
...
class ArbitrageCalculator:
    ...
    def _group_odds_data(self, odds_data: List[Dict[str, Any]]) -> Dict[str, List[Dict]]:
    ...
    async def _calculate_two_way_arbitrage(self, odds_list: List[Dict[str, Any]]) -> List[ArbitrageOpportunity]:
        ...
        try:
                    ...
                    if self._are_opposite_outcomes(odds1, odds2):
                        ...
                        if arb_result and arb_result['profit_percentage'] > 0:
                            opportunity = ArbitrageOpportunity(
                                ...
                                execution_risk=self._calculate_execution_risk([odds1, odds2]),
                                ...
                                implied_probabilities=[1/odds1['odds'], 1/odds2['odds']],
                                ...
                                source_quality=min(odds1.get('quality', 0.8), odds2.get('quality', 0.8)),
            ...
            logger.error(f"Two-way arbitrage calculation failed: {str(e)}")
            return []
    
    def _calculate_two_way_math(self, odds1: Dict[str, Any], odds2: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            confidence = min(
                odds1.get('quality', 0.8),
                odds2.get('quality', 0.8)
            ) * (1 - abs(profit_percentage) / 100)  # Higher profits are more suspicious
            ...
            return {
                ...
                'stake1': stake1,
                'stake2': stake2,
                'optimal_stakes': {odds1['sportsbook']: stake1, odds2['sportsbook']: stake2},
            ...
            logger.error(f"Two-way arbitrage math failed: {str(e)}")
            return None
    
    async def _calculate_three_way_arbitrage(self, odds_list: List[Dict[str, Any]]) -> List[ArbitrageOpportunity]:
        ...
        try:
            ...
            sportsbooks = [best_odds[outcome]['sportsbook'] for outcome in outcome_types]
            ...
            if arbitrage_percentage < 1.0:
                # Arbitrage exists
                total_stake = 100.0
                stakes = [total_stake / (arbitrage_percentage * odds) for odds in odds_values]
                
                guaranteed_profit = min(
                    stakes[i] * odds_values[i] - total_stake 
                    for i in range(3)
                )
                ...
                if guaranteed_profit > 0:
                    opportunity = ArbitrageOpportunity(
                        ...
                        execution_risk=self._calculate_execution_risk(list(best_odds.values())),
                        liquidity_risk=self._calculate_liquidity_risk(list(best_odds.values())),
                        timing_risk=self._calculate_timing_risk(list(best_odds.values())),
                        ...
                        source_quality=min(odds.get('quality', 0.8) for odds in best_odds.values()),
            ...
            logger.error(f"Three-way arbitrage calculation failed: {str(e)}")
            return []
    
    async def _calculate_cross_market_arbitrage(self, odds_list: List[Dict[str, Any]]) -> List[ArbitrageOpportunity]:
    ...
    async def _calculate_triangular_arbitrage(self, odds_list: List[Dict[str, Any]]) -> List[ArbitrageOpportunity]:
    ...
    async def _calculate_synthetic_arbitrage(self, odds_list: List[Dict[str, Any]]) -> List[ArbitrageOpportunity]:
    ...
    def _are_opposite_outcomes(self, odds1: Dict[str, Any], odds2: Dict[str, Any]) -> bool:
    ...
    def _calculate_execution_risk(self, odds_list: List[Dict[str, Any]]) -> float:
        ...
        for odds in odds_list:
            # Time risk
            time_since_update = (datetime.utcnow() - odds.get('timestamp', datetime.utcnow())).total_seconds()
            time_risk = min(time_since_update / 300, 1.0)  # Risk increases over 5 minutes
            ...
            risk_factors.append(time_risk * 0.4 + reliability_risk * 0.3 + volatility * 0.3)
    ...
    def _calculate_liquidity_risk(self, odds_list: List[Dict[str, Any]]) -> float:
        ...
        for odds in odds_list:
            ...
            liquidity_score = min(market_volume / 10000, 1.0) * min(max_stake / 10000, 1.0)
...
class MarketInefficiencyDetector:
    ...
    def _initialize_behavioral_patterns(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'home_bias': {
                'public_home_premium': 0.15,
                'sharp_adjustment': -0.05
            },
            'favorite_bias': {
    ...
    async def detect_market_inefficiencies(
        ...
        try:
            for market in market_data:
                ...
                pricing_inefficiencies = await self._detect_pricing_errors(market, historical_data)
                ...
                steam_inefficiencies = await self._detect_steam_moves(market, historical_data)
                inefficiencies.extend(steam_inefficiencies)
                
                # 5. Reverse Line Movement Detection
                reverse_line_inefficiencies = await self._detect_reverse_line_movement(market)
            ...
            logger.error(f"Market inefficiency detection failed: {str(e)}")
            return []
    ...
    async def _detect_pricing_errors(
        ...
        try:
            ...
            z_score = self._calculate_z_score(market_prob, fair_prob, market.get('sample_size', 100))
            
            if (mispricing_percentage > 5.0 and  # At least 5% mispricing
                abs(z_score) > self.statistical_models['z_score_threshold']):
                
                # Determine if it's a value bet
                if market_price > consensus_fair_value:  # Market overpricing (value bet)
                    ...
                    if expected_value > 0.02:  # At least 2% edge
                        inefficiency = MarketInefficiency(
                            ...
                            z_score=z_score,
                            confidence_interval=self._calculate_confidence_interval(fair_prob),
                            ...
                            kelly_fraction=self._calculate_kelly_fraction(fair_prob, market_price),
                            recommended_stake=0.0,  # Will be calculated by risk engine
                            max_stake=market.get('max_stake', 1000),
                            model_uncertainty=np.std(list(fair_values.values())) / consensus_fair_value,
            ...
            logger.error(f"Pricing error detection failed: {str(e)}")
            return []
    ...
    async def _calculate_fair_value(
        ...
        try:
            ...
            if historical_data:
                closing_odds = [h.get('closing_odds') for h in historical_data if h.get('closing_odds')]
                if closing_odds:
            ...
            logger.error(f"Fair value calculation failed: {str(e)}")
            return {}
    
    async def _detect_information_lag(self, market: Dict[str, Any]) -> List[MarketInefficiency]:
        ...
        try:
                ...
                if info_lag > 300:  # More than 5 minutes lag
                    ...
                    if info_impact > 0.3:  # Material information
                        inefficiency = MarketInefficiency(
                            ...
                            sportsbook=market['sportsbook'],
                            market_price=market.get('odds', 0),
                            fair_value=market.get('pre_info_odds', market.get('odds', 0)),
            ...
            logger.error(f"Information lag detection failed: {str(e)}")
            return []
    
    async def _detect_behavioral_biases(self, market: Dict[str, Any]) -> List[MarketInefficiency]:
        ...
        try:
            ...
            if market.get('is_home_team'):
                ...
                if public_percentage > 65:  # Heavy public support for home team
                    bias_premium = self.behavioral_patterns['home_bias']['public_home_premium']
                    
                    inefficiency = self._create_bias_inefficiency(
                        market, 'home_bias', bias_premium, 
            ...
            if market.get('is_favorite'):
                ...
                if public_percentage > 70:  # Heavy public support for favorite
                    bias_premium = self.behavioral_patterns['favorite_bias']['public_favorite_premium']
                    
                    inefficiency = self._create_bias_inefficiency(
                        market, 'favorite_bias', bias_premium,
            ...
            if odds in self.behavioral_patterns['round_number_bias']['psychological_levels']:
                bias_penalty = self.behavioral_patterns['round_number_bias']['round_odds_penalty']
                
                inefficiency = self._create_bias_inefficiency(
                    market, 'round_number_bias', bias_penalty,
    ...
    def _create_bias_inefficiency(
        ...
        try:
            ...
            logger.error(f"Bias inefficiency creation failed: {str(e)}")
            return None
    ...
    async def _detect_steam_moves(
        ...
        try:
            ...
            recent_odds = [h.get('odds') for h in historical_data[-10:] if h.get('odds')]
            ...
            if abs(line_movement) > 0.05:  # At least 5% movement
                ...
                if ((line_movement < 0 and public_percentage > 60) or  # Line shortening, public on this side
                    (line_movement > 0 and public_percentage < 40)):   # Line lengthening, public on other side
                    
                    steam_strength = abs(line_movement) * (abs(public_percentage - 50) / 50)
                    ...
                    inefficiency = MarketInefficiency(
                        ...
                        sharp_money_percentage=market.get('sharp_percentage'),
                        line_movement_direction='against_public' if line_movement < 0 else 'with_public',
            ...
            logger.error(f"Steam move detection failed: {str(e)}")
            return []
    
    async def _detect_reverse_line_movement(self, market: Dict[str, Any]) -> List[MarketInefficiency]:
        """Detect reverse line movement"""
        # Implementation for reverse line movement detection
        return []
    
    def _calculate_z_score(self, sample_prob: float, population_prob: float, sample_size: int) -> float:
    ...
    def _calculate_confidence_interval(self, probability: float, confidence: float = 0.95) -> Tuple[float, float]:
        ...
        margin_error = z_score * np.sqrt(probability * (1 - probability) / 100)  # Assuming sample size 100
...
class UltraArbitrageEngine:
    ...
    async def scan_for_opportunities(
        ...
        try:
            ...
            arbitrage_opportunities = await self.arbitrage_calculator.detect_arbitrage_opportunities(market_data)
            
            # Detect market inefficiencies
            market_inefficiencies = await self.inefficiency_detector.detect_market_inefficiencies(
                market_data, historical_data
            )
            
            # Update performance metrics
            self.performance_metrics["opportunities_detected"] += len(arbitrage_opportunities) + len(market_inefficiencies)
            ...
            return {
                ...
                "total_opportunities": len(arbitrage_opportunities) + len(market_inefficiencies),
