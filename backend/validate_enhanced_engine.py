...
def validate_neuromorphic_network():
    ...
    try:
        # Create network
        net = EnhancedNeuromorphicNetwork(input_dim=10, hidden_dims=[64, 32], output_dim=5)
        ...
        checks = {
            ...
            "Network criticality bounded": 0 <= metrics.get('network_criticality', 0) <= 1,
            "No NaN values": not torch.isnan(output).any(),
            "Finite membrane potentials": all(torch.isfinite(v).all() for v in metrics.get('membrane_dynamics', {}).get('mean_voltage', []))
        }
        ...
        hh_checks = {
            ...
            "Gating variables bounded": all(0 <= var <= 1 for var in [m_new, h_new, n_new]),
            "Membrane dynamics stable": abs(v_new - v) < 50  # Reasonable voltage change
        }
        
        hh_passed = sum(hh_checks.values())
        print(f"  Hodgkin-Huxley validation: {hh_passed}/{len(hh_checks)} checks passed")
...
def validate_mamba_state_space():
    ...
    try:
        ...
        checks = {
            ...
            "State magnitude finite": torch.isfinite(torch.tensor(metrics['state_magnitude'])),
        ...
        print(f"    {'✓' if stability_check else '✗'} Stability condition (max|λ| = {max_eigenvalue:.3f} < 1.0)")
...
def validate_topological_network():
    ...
    try:
        ...
        checks = {
            ...
            "Non-negative Betti numbers": all(b >= 0 for b in metrics.get('betti_numbers', {}).values()),
            "Persistence measure finite": torch.isfinite(torch.tensor(metrics.get('persistence_measure', 0))),
            "Euler characteristic computed": 'euler_characteristic' in metrics,
            "Topological complexity bounded": metrics.get('topological_complexity', 0) >= 0
        }
        ...
        print(f"    ���� Betti numbers: β₀={betti.get('H0', 0)}, β₁={betti.get('H1', 0)}, β₂={betti.get('H2', 0)}")
...
def validate_riemannian_network():
    ...
    try:
        ...
        checks = {
            ...
            "Curvature finite": torch.isfinite(torch.tensor(metrics['riemannian_curvature'])),
            "Manifold distance non-negative": metrics['mean_manifold_distance'] >= 0,
            "Metric determinant positive": metrics['metric_determinant'] > 0,
            "Geodesic displacement finite": torch.isfinite(torch.tensor(metrics['geodesic_displacement'])),
        ...
        print(f"    {'✓' if positive_definite else '✗'} Metric tensor positive definite")
...
def validate_enhanced_engine():
    ...
    try:
        ...
        checks = {
            ...
            "Mathematical guarantees present": isinstance(prediction.mathematical_guarantees, dict),
            ...
            "Betti numbers valid": all(isinstance(b, int) and b >= 0 for b in prediction.persistent_betti_numbers.values()),
