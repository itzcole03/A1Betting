...
class TaskQueue:
                ...
                if result:
                    ...
                    if lock_acquired:
                        ...
                        if task_data:
                            ...
                            logger.debug(f"Dequeued task {task_id} by worker {worker_id}")
                            return task
                        else:
    ...
    async def store_result(self, result: TaskResult):
        ...
        try:
            ...
            logger.error(f"Failed to store result for task {result.task_id}: {str(e)}")
    ...
    async def get_queue_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for priority in TaskPriority:
                ...
                if count > 0:
                    oldest = await self.redis_client.zrange(queue_key, 0, 0, withscores=True)
                    newest = await self.redis_client.zrange(queue_key, -1, -1, withscores=True)
                    
                    if oldest:
                        oldest_time = oldest[0][1]
                        stats["oldest_task_age"] = max(stats["oldest_task_age"], time.time() - oldest_time)
                    
                    if newest:
                        newest_time = newest[0][1]
                        stats["newest_task_age"] = min(stats["newest_task_age"], time.time() - newest_time)
...
class TaskWorker:
    ...
    def __init__(self, worker_id: str, concurrency: int = 4):
        ...
        self.process_executor = ProcessPoolExecutor(max_workers=max(1, mp.cpu_count() // 2))
    ...
    async def start(self):
        ...
        logger.info(f"Starting task worker {self.worker_id} with concurrency {self.concurrency}")
    ...
    async def _execute_task(self, task: TaskDefinition, worker_thread_id: str) -> TaskResult:
    ...
    def _model_training_task(self, model_name: str, training_data: Dict[str, Any], **kwargs):
        """Model training task"""
        ...
        try:
            ...
            for event_id in event_ids:
                ...
                prediction = await ultra_ensemble_engine.predict(
                    features=features,
                    context=kwargs.get('context', 'pre_game')
                )
    ...
    async def _risk_analysis_task(self, portfolio: Dict[str, Any], **kwargs):
        ...
        try:
            ...
            logger.error(f"Risk analysis task failed: {str(e)}")
            return {"status": "failed", "error": str(e)}
    ...
    async def _arbitrage_scan_task(self, market_data: List[Dict[str, Any]], **kwargs):
        ...
        try:
            ...
            return {
                "status": "success",
                "arbitrage_opportunities": len(opportunities["arbitrage_opportunities"]),
                "market_inefficiencies": len(opportunities["market_inefficiencies"]),
                "best_profit": max([
                    arb.profit_percentage for arb in opportunities["arbitrage_opportunities"]
                ], default=0),
                "timestamp": datetime.utcnow().isoformat()
            }
            
        except Exception as e:
            logger.error(f"Arbitrage scan task failed: {str(e)}")
            return {"status": "failed", "error": str(e)}
    ...
    def _analytics_computation_task(self, metrics: List[str], **kwargs):
        ...
        try:
            ...
            logger.error(f"Analytics computation task failed: {str(e)}")
            return {"status": "failed", "error": str(e)}
    ...
    async def get_system_stats(self) -> Dict[str, Any]:
        ...
        for worker_id, worker in self.workers.items():
            worker_stats[worker_id] = {
                ...
                "success_rate": ((worker.tasks_processed - worker.tasks_failed) / max(worker.tasks_processed, 1)) * 100,
                "avg_execution_time": worker.total_execution_time / max(worker.tasks_processed, 1),
