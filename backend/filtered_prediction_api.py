...
def predict_filtered(
    ...
    min_confidence: Optional[float] = Query(None, description="Minimum confidence required"),
    ...
    if max_risk is not None and (1 - response.confidence) > max_risk:
        response.explanation += f" Prediction filtered out: risk {(1 - response.confidence)} > {max_risk}."
    return response
